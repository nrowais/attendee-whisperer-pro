...
export const getAirportBoard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: flights, error } = await supabase
      .from("flights")
      .select("*")
      .or(`arrival_time.gte.${start},arrival_time.lte.${end},departure_time.gte.${start},departure_time.lte.${end}`)
...
export const syncFlightStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return { ok: true, flightId: data.flightId, live };
  });

/**
 * تحديث جميع رحلات اليوم.
 */
    ...
    for (const f of flights ?? []) {
      try {
        results.push({
          flightId: f.id,
          ok: true,
          result: await syncFlightStatus({ data: { flightId: f.id } }),
        });
      } catch (e: any) {
